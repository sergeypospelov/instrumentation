package com.huawei.utbot.instrumentation.instrumentation.coverage

import com.huawei.utbot.common.withRemovedFinalModifier
import com.huawei.utbot.instrumentation.ConcreteExecutor
...
import com.huawei.utbot.instrumentation.util.CastProbesArrayException
import com.huawei.utbot.instrumentation.util.ChildProcessError
import com.huawei.utbot.instrumentation.util.InstrumentationException
import com.huawei.utbot.instrumentation.util.NoProbesArrayException
...
object CoverageInstrumentation : Instrumentation<Result<*>> {
    ...
    override fun invoke(
        ...
        val visitedLinesField = clazz.fields.firstOrNull { it.name == probesFieldName }
            ?: throw NoProbesArrayException(clazz.fields.joinToString { it.name })
...
fun ConcreteExecutor<Result<*>, CoverageInstrumentation>.collectCoverage(clazz: Class<*>): CoverageInfo {
    ...
    return this.request(collectCoverageCommand) {
        when (it) {
            ...
            is Protocol.ErrorCommand -> throw ChildProcessError(it.exception)
